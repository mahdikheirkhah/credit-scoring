...
class POSCashAggregator(FeatureAggregator):
    ...
    def aggregate(self, pos_df: pd.DataFrame) -> pd.DataFrame:
        try:
            logger.info("Aggregating POS_CASH_balance.csv (Internal Monthly History)...")
            ...
            df_pos['POS_IS_ACTIVE'] = (df_pos['NAME_CONTRACT_STATUS'] == 'Active').astype(int)
            df_pos['POS_IS_COMPLETED'] = (df_pos['NAME_CONTRACT_STATUS'] == 'Completed').astype(int)
            
            # --- AGGREGATION: Flatten the history to the Client Level ---
            aggregations = {
                'MONTHS_BALANCE': ['max', 'min', 'size'], # Recency and length of history
                'SK_DPD_DEF': ['max', 'mean'], # The absolute worst they've ever been
                'POS_IS_LATE': ['sum', 'mean'], # Total months late, and % of months late
            ...
            logger.info(f"Successfully engineered POS Cash proxies. Shape: {agg_df.shape}")
            return agg_df.reset_index()
...
class CreditCardAggregator(FeatureAggregator):
    ...
    def aggregate(self, cc_df: pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            df_cc['SAFE_LIMIT'] = df_cc['AMT_CREDIT_LIMIT_ACTUAL'].replace(0, float('nan'))
            df_cc['CC_UTILIZATION'] = df_cc['AMT_BALANCE'] / df_cc['SAFE_LIMIT']
            df_cc['CC_ATM_DRAWING_RATIO'] = df_cc['AMT_DRAWINGS_ATM_CURRENT'] / df_cc['SAFE_LIMIT']
            
            # Guardrail: Cap astronomical ratios (e.g., caused by $1 limits + fees)
            df_cc['CC_UTILIZATION'] = df_cc['CC_UTILIZATION'].clip(upper=1.5)
            df_cc['CC_ATM_DRAWING_RATIO'] = df_cc['CC_ATM_DRAWING_RATIO'].clip(upper=1.5)
            ...
            card_level_aggs = {
                ...
                'AMT_DRAWINGS_ATM_CURRENT_WAS_MISSING': ['sum'], # Months this card was inactive
            }
            # We group by both to keep CURR available for Step 2
            card_agg = df_cc.groupby(['SK_ID_CURR', 'SK_ID_PREV']).agg(card_level_aggs)
            card_agg.columns = pd.Index([f"CARD_{e[0]}_{e[1].upper()}" for e in card_agg.columns.tolist()])
            ...
            client_level_aggs = {
                ...
                'CARD_CC_UTILIZATION_MAX': ['max'], # The HIGHEST utilization across ALL their cards
                'CARD_CC_UTILIZATION_MEAN': ['mean'],
                'CARD_CC_ATM_DRAWING_RATIO_MAX': ['max'],
                'CARD_CC_IS_LATE_SUM': ['sum'],
                'CARD_CC_IS_SEVERE_LATE_SUM': ['sum'],
                'CARD_AMT_DRAWINGS_ATM_CURRENT_WAS_MISSING_SUM': ['sum'] # Total inactive months
            }
            
            final_agg = card_agg.groupby('SK_ID_CURR').agg(client_level_aggs)
            final_agg.columns = pd.Index([f"CLIENT_{e[0]}_{e[1].upper()}" for e in final_agg.columns.tolist()])
            
            logger.info(f"Successfully engineered Two-Step Credit Card proxies. Shape: {final_agg.shape}")
...
class InstallmentsAggregator(FeatureAggregator):
    ...
    def aggregate(self, inst_df: pd.DataFrame) -> pd.DataFrame:
        try:
            logger.info("Aggregating installments_payments.csv (Two-Step Aggregation)...")
            ...
            df_inst['DAYS_ENTRY_PAYMENT'] = df_inst['DAYS_ENTRY_PAYMENT'].fillna(df_inst['DAYS_INSTALMENT'] + 365)
            ...
            df_inst['SAFE_INSTALMENT'] = df_inst['AMT_INSTALMENT'].replace(0, float('nan'))
            df_inst['PAYMENT_FRACTION'] = df_inst['AMT_PAYMENT'] / df_inst['SAFE_INSTALMENT']
            ...
            df_inst['IS_UNDERPAID'] = (df_inst['PAYMENT_FRACTION'] < 0.99).astype(int) # 0.99 handles rounding errors
            
            # ==========================================================
            # STEP 1: AGGREGATE TO LOAN LEVEL (SK_ID_PREV)
            # ==========================================================
            loan_level_aggs = {
                'NUM_INSTALMENT_VERSION': ['nunique'], # Count how many times the loan terms were changed
            ...
            loan_agg = df_inst.groupby(['SK_ID_CURR', 'SK_ID_PREV']).agg(loan_level_aggs)
            loan_agg.columns = pd.Index([f"LOAN_{e[0]}_{e[1].upper()}" for e in loan_agg.columns.tolist()])
            ...
            # ==========================================================
            client_level_aggs = {
                'SK_ID_PREV': ['count'], # Total previous loans with installment history
                'LOAN_NUM_INSTALMENT_VERSION_NUNIQUE': ['sum'], # Total term restructures across all loans
                'LOAN_PAYMENT_DELAY_MAX': ['max'], # The absolute worst delay across any loan
                'LOAN_PAYMENT_DELAY_MEAN': ['mean'],
                'LOAN_PAYMENT_FRACTION_MIN': ['min'], # The worst underpayment fraction across any loan
            ...
            final_agg.columns = pd.Index([f"CLIENT_INST_{e[0]}_{e[1].upper()}" for e in final_agg.columns.tolist()])
            
            logger.info(f"Successfully engineered Two-Step Installment proxies. Shape: {final_agg.shape}")
            return final_agg.reset_index()
...
class PreviousApplicationAggregator(FeatureAggregator):
    ...
    def aggregate(self, prev_df: pd.DataFrame) -> pd.DataFrame:
        try:
            logger.info("Aggregating previous_application.csv (Internal Application History)...")
            ...
            df_prev['AREA_WAS_NEGATIVE'] = (df_prev['SELLERPLACE_AREA'] == -1).astype(int)
            df_prev['SELLERPLACE_AREA'] = df_prev['SELLERPLACE_AREA'].replace(-1, float('nan'))
            
            # --- 2. DEEPSEEK UPGRADE: PRE-AGGREGATION MISSINGNESS FLAGS ---
            # Capture missingness as a behavior BEFORE aggregating
            df_prev['HAS_DISBURSEMENT_INFO'] = df_prev['DAYS_FIRST_DRAWING'].notna().astype(int)
            df_prev['HAS_DOWN_PAYMENT'] = df_prev['AMT_DOWN_PAYMENT'].notna().astype(int)
            df_prev['HAS_INTEREST_RATE'] = df_prev['RATE_INTEREST_PRIMARY'].notna().astype(int)
            ...
            df_prev['APPLICATION_CREDIT_RATIO'] = df_prev['AMT_APPLICATION'] / df_prev['SAFE_CREDIT']
            df_prev['APPLICATION_CREDIT_RATIO'] = df_prev['APPLICATION_CREDIT_RATIO'].clip(upper=5.0)
            ...
            logger.info(f"Successfully engineered Previous Application proxies. Shape: {agg_df.shape}")
            return agg_df.reset_index()
