...
class BaselinePreprocessor:
    ...
    def __init__(self, random_state: int = None) -> None:
        ...
        try:
            ...
            self.cols_to_drop = []
            logger.info(
                f"Initialized BaselinePreprocessor with random_state={self.random_state}"
            )
    ...
    def filter_features(self, X: pd.DataFrame, fit: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            if fit:
                logger.info(
                    "Dynamically learning features to drop (zero-variance & redundant)..."
                )

                # 1. Redundant Building Features
                building_redundant = [
                    c
                    for c in X_cleaned.columns
                    if c.endswith("_AVG") or c.endswith("_MODE")
                ]
                ...
                logger.info(
                    f"Memorized {len(self.cols_to_drop)} columns to drop globally."
                )
            ...
            numeric_transformer = Pipeline(
                steps=[
                    (
                        "imputer",
                        SimpleImputer(
                            strategy=CONFIG.preprocess.num_impute_strategy,
                            add_indicator=True,
                        ),
                    ),
                    ("scaler", RobustScaler()),
    ...
    def fit_transform(self, X: pd.DataFrame, y: pd.Series = None) -> pd.DataFrame:
        ...
        try:
            ...
            self.feature_names = [
                re.sub(r'[\[\]{},:" ]', "_", name) for name in raw_names
            ]
